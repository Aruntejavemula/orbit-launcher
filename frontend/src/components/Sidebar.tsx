...
function Initials({ name, avatarUrl }: { name: string; avatarUrl?: string | null }) {
  ...
  return (
    <div
      className="h-10 w-10 rounded-full flex items-center justify-center font-display text-base font-semibold"
      style={{ background: "var(--hero-2)", color: "var(--text)" }}
    >
      {text.toUpperCase()}
    </div>
  );
    ...
    >
      ...
      >
        <span className="relative">
          <Initials name={user?.name ?? "?"} avatarUrl={user?.avatar_url} />
